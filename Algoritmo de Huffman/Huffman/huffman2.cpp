...
typedef struct Nodo
{
	char caracter;
	int frecuencia;
	char codificacion[8]; //guardamos los octetos de de nuestra frase
	struct Nodo *sig;
	struct Nodo *ant;
...
Lista* Crearnodo(char dato, int frecuencia,Arbol* apuntadorarbol); //crea un nodo en nuestra lista
Lista* Altainicio(Lista* Inicio, char dato, int frecuencia,Arbol* apuntadorarbol); //pone nuestro nodo al principio de la lista
Lista* Altap(Lista* Inicio, int pos,char dato,int frecuencia,Arbol* apuntadorarbol); //inserta el nodo en una posicion especifica de la lista
Lista* Altafinal(Lista* Inicio,char dato,int frecuencia,Arbol* apuntadorarbol); //inserta el nodo al final de la lista

//Todo lo que se ocupa para la coidficacion de Huffman
Lista* Generarfrecuencias(char *frase); //contamos los carcteres de nuestra frase y generamos las frecuencias de cada caracter
Lista* Ordenar(Lista* Inicio); //ordenamos nuestra Lista de acuerdo a las frecuencias (selction sort)
void Mostrar(Lista* Inicio); //Mostramos la lista
int Tam(Lista* Inicio); //Recorre la lista y obtiene el tamaño de la misma
...
void Generarlistaarbol(Arbol *Raiz, Lista **Inicio); //guardamos nuestro arbol en una lista
void Creararchivoarbol(Arbol *Raiz, Lista *Inicio); //creamos un archivo para guardar el arbol de manera preorden y usarlo en el decifrado
void Codificar(Arbol *Raiz, char code[],Lista **Inicio, int i); //generamos los codigos de frecuencia variable de huffman recorriendo el arbol
void Mostrarcodigo(Lista *Inicio); //Mostramos los codigos conseguidos que guardamos en la lista
void Codbinario(Lista *Codigos, char oracion[],char codb[],char codc[]);//la cadena final con todos los codigos la dividimos en octetos y lo comprimimos en decimales para cifrar el mensaje
...
Lista* Conseguirarbol(); //lee el archuvo que guardamos del arbol y lo mete dentro de una nueva lista
Arbol2* Altaarbol(Lista **Inicio, Arbol* Arbol); //recorre la lista y construye nuestro arbol preorden
Arbol2* Generararbolp(Arbol* Raiz, Lista** Inicio); //
void Leerarchivo(char palabra[]);// lee el archivo codificado
void Binarioleido(unsigned char cod[],unsigned char codbinario[]); //genera los codigos codigos binarios de nuestro codificado
void Traermensaje(unsigned char codleido[],Lista* codc,char mensaje[]); //regresa nuestra frase original
...
void Codbinario(Lista *Codigos, char oracion[],char codb[],char codc[])
{
		...
		Lista *apuntador;
		apuntador=NULL;
		//va a leer nuestra frase original
		while(oracion[i]!='\0')
		{
			aux=toupper(oracion[i]); //vuelve a generar en mayusculas nuestra oracion y la guarda en un auxiliar
			apuntador=Codigos; //apuntamos a los codigos
			...
			{
				codb[k]=apuntador->codificacion[j]; //concatenamos la frase con base de los codigos obtenidos
				j++;
				k++;
			}
			i++;
		}
		codb[k]='\0'; //le damos un termino a nuestra frase
		i=0;
		printf("Longitud de la frase: %d\n",strlen(oracion)); //enseñamos la longitud de nuestra frase original
		printf("Longitud de la frase en binario: %d\n",strlen(codb)); //enseñamos la longitud de nuestra cadena en binario
		while(codb[i]!='\0')
		{
			printf("%c",codb[i]); //imprimimos la cadena en binario
			i++;
		}
		printf("\n");
		mod=strlen(codb)%8; //vemos si el recidui de la cadena binaria es 0
		cantc=strlen(codb); //obtenemos la cantidad de la frase en binario
		cant=strlen(codb)/8; //dividimos la frase entre 8
		k=0;
		if(mod!=0) //si la frase es divisible entre 8
		{
			auxcant=((cantc)+((8)-(cantc-(cant*8)))); //esto es para llenar de 0 lo que necesitamos para que ya sea divisible entre 8
			cdec[k]=0;
			for(h=0;h<auxcant;h++)
			{
				if(l==8)
				...
				if(h>cantc) //rellenamos los 0 que hagan falta para que nos imprima una frase ascci correcta
				{
					auxconv='0';
				}
				else
				...
				printf("%d- %d\n",h+1,conv);
				cdec[k]+=((conv)*(p[l])); //convierte nuestra cadena de 8 bits a un decimal
				l++;
			}
			cdec[k+1]='\0';
			l=0;
			while(cdec[l]!='\0')
			{
				printf("%d ",cdec[l]); //imprimimos nuestro arreglo que convierte a decimal nuestros octetos
				l++;
			}
			system("PAUSE");
		}
		...
		while(cdec[k]!='\0')
		{
			codc[k]=cdec[k]; //pasamos nuestro arreglo decimal a la variable que lo va a guardar
			k++;
		}
		codc[l]='\0';
}
...
Lista* Conseguirarbol()
{
		...
		else
		{
			...
			while(!feof(Archivo))
			{
				...
				if(caux>=48 && caux<=57) //rango para espacios
				{
					caux='_';
				}
...
void Binarioleido(unsigned char cod[],unsigned char codbinario[])
{
		...
		while(cod[i]!='\0')
		{
			codd[i]=cod[i]; //le pasamos lo que hay en nuestro archivo a nuestra variable
			i++;
		}
		codd[i]='\0'; //finalizamos la cadena
		i=0;
		while(codd[i]!='\0')
		{
			printf("\n%d:\t",codd[i]);
			for(k=8;k>0;k--)
			{
				codcar[k]=codd[i]%2;
				codd[i]=codd[i]/2;
				printf("%d ",codcar[k]); //imprime la cadena de 8 bits en binario
			}
			for(l=1;l<9;l++)
			{
				if(codcar[l]==0)
				{
					codbinario[j]='0'; //pasamos los 8 bits de binario a caracter
				}
				else
				{
					codbinario[j]='1';
		...
		codbinario[j]='\0'; //damos el fin de linea
		printf("\n\nCadena unida: \n%s\n",codbinario);
}
...
void Traermensaje(unsigned char codleido[],Lista* codc,char mensaje[])
{
		...
		Lista *apuntador;
		apuntador=codc;
		//leemos la cadena hatsa lo que logramos codificar y sin los 0 que agregamos
		while(codleido[i]!='\0' && t!=1)
		{
			j=0;
			if(t==0) //todavia no llegasmos a los 0
			{
				if(ap==0)//ve si existe un match
				{
					l=0;
					aux[l]=codleido[i];
				}
				...
				while(apuntador!=NULL && match!=1)
				{
					...
					if(tamaux==tamc)
					{
						if(strcmp(aux,apuntador->codificacion)==0) //comparamos si la codificacion de nuestro apuntador y nuestra cadena que tiene el codigo son iguales
						{
							match++;
						}
					}
				...
				n=1;
				if(match==1) //si encuentra un codigo que sea igual a la cadena concatenada lo agrega al mensaje final
				{
					apuntador=codc;
					while(n!=pos)
					{
